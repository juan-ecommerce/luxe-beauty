    ...
    <section ref={ref} id="inicio" className="relative h-screen min-h-[640px] flex items-center justify-center overflow-hidden bg-obsidian">
      ...
      <div className="relative z-10 text-center px-6 max-w-5xl mx-auto">
        ...
        <motion.div {...fadeUp(0)} className="mb-10 flex justify-center">
          <div className="bg-white/97 px-10 py-6 border border-gold-light/20 shadow-[0_0_60px_rgba(212,175,55,0.15)]">
            <img src={luxeLogo} alt="Luxe Beauty Coworking"
              style={{ height: 'clamp(5rem, 10vw, 9rem)', width: 'auto' }} />
          </div>
        </motion.div>
        ...
        {/* Description — co-working emphasis */}
        <motion.p
          {...fadeUp(0.6)}
          className="font-body text-white/65 max-w-2xl mx-auto leading-relaxed"
          style={{ fontSize: 'clamp(0.95rem, 1.5vw, 1.15rem)' }}
        >
          {t('hero.desc')}
        </motion.p>

        {/* Co-working tag pills */}
        <motion.div
          {...fadeUp(0.75)}
          className="flex flex-wrap justify-center gap-3 mt-8"
        >
          {['hero.tag1', 'hero.tag2', 'hero.tag3'].map(key => (
            <span key={key}
              className="font-body text-xs text-gold-light/80 tracking-[0.2em] uppercase
                border border-gold-light/25 px-4 py-2">
              {t(key)}
            </span>
          ))}
        </motion.div>
